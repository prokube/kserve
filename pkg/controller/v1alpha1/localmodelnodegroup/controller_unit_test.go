...
func TestCreateLocalModelAgentDaemonSet(t *testing.T) {
	...
	t.Run("validate daemonset creation with correct configuration", func(t *testing.T) {
		...
		ds, err := createLocalModelAgentDaemonSet(nodeGroup, localModelConfig, pvcName)
	...
	t.Run("validate daemonset with different configuration", func(t *testing.T) {
		...
		ds, err := createLocalModelAgentDaemonSet(nodeGroup, differentConfig, "different-pvc")
		require.NoError(t, err, "Expected no error when creating DaemonSet")
	...
	t.Run("return error for invalid resource quantities", func(t *testing.T) {
		invalidConfig := v1beta1.LocalModelConfig{
			LocalModelAgentImage:           "kserve/agent:latest",
			LocalModelAgentImagePullPolicy: "IfNotPresent",
			LocalModelAgentCpuRequest:      "not-a-quantity",
			LocalModelAgentMemoryRequest:   "200Mi",
			LocalModelAgentCpuLimit:        "500m",
			LocalModelAgentMemoryLimit:     "500Mi",
		}

		ds, err := createLocalModelAgentDaemonSet(nodeGroup, invalidConfig, pvcName)
		require.Error(t, err, "Expected error for invalid CPU request")
		require.Nil(t, ds, "Expected no DaemonSet to be returned")
		require.Contains(t, err.Error(), "localModelAgentCpuRequest", "Error should reference the invalid field")
	})
}
...
func TestReconcileDaemonSet(t *testing.T) {
	...
	t.Run("update DaemonSet when it already exists with different spec", func(t *testing.T) {
		...
		existingDS, err := createLocalModelAgentDaemonSet(*nodeGroup, *localModelConfig, "old-pvc-name")
		require.NoError(t, err, "Expected no error when creating existing DaemonSet")
	...
	t.Run("no update when DaemonSet hasn't changed", func(t *testing.T) {
		...
		existingDS, err := createLocalModelAgentDaemonSet(*nodeGroup, *localModelConfig, pvcName)
		require.NoError(t, err, "Expected no error when creating existing DaemonSet")
		existingDS.Namespace = constants.KServeNamespace
